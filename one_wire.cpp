...
bool One_wire::search_rom_find_next() {
	...
	if (!reset_check_for_device()) {
		//Serial.print("Failed to reset one wire bus\n");
		return false;
	} else {
		if (_last_device) {
		...
		while (rom_bit_index <= 64) {
			...
			if (bitA & bitB) {
				...
				rom_bit_index = 0xFF;
				//Serial.print("Data read error - no devices on bus?\r\n");
			} else {
				if (bitA | bitB) {
		...
		if (rom_bit_index != 0xFF) {
			#ifdef _LIST_ROMS
			//Serial.print("Found");
			//Serial.print(search_ROM[0]);
			//Serial.print(search_ROM[1]);
			//Serial.print(search_ROM[2]);
			//Serial.print(search_ROM[3]);
			//Serial.print(search_ROM[4]);
			//Serial.print(search_ROM[5]);
			//Serial.print(search_ROM[6]);
			//Serial.print(search_ROM[7]);
			#endif

			if (rom_checksum_error(_search_ROM)) {// Check the CRC
				//Serial.print("failed crc\r\n");
				return false;
			}
			rom_address_t address{};
	...
	if (ram_checksum_error())
		...
		switch (FAMILY_CODE) {
			...
			case FAMILY_CODE_DS18S20:
				...
				break;
			default:
				//Serial.print("Unsupported device family\n");
				break;
		}
